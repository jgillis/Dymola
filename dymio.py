...
class DymReader:
	...
	def get(self,name):
		...
		if k==0:
			...
			k=1		
		description=dym.description[i]
		unit=''
		if description:
			m=self.pat.match(description)
			if m:
				unit=m.group(0)
		if k==0:
			val=sign(j)*dym.data[k][0,abs(j)]
		else:
			val=sign(j)*dym.data[k][:,abs(j)]

		return {'type':dym.dataInfo[i,0],'value': val,'i':i,'description':dym.description[i],'unit':unit}
	def has(self,name):
		try:
			self.dym.get_variable_index(name)
			return 0
		except:
			return 0
